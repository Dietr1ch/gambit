...
namespace Gambit
{
  ...
  {  
    ...
    //// NEW BEAM-DUMP CONTENT Forward Declarations
    ...
    void get_all_SubGeVBeamDump_MB_signal_yields(std::vector<double>&, const Model_analysis_info&, const Spectrum&);
  ...
  /// A function for filling the analysis_info_map for the <modelname> model.
...
/// Results for the SubGeVBeamDump_MB model
    ...
    {
      using namespace Pipes::<modelname>_results;
...
/// Fill the input vector with the total SubGeVBeamDump_MB signal prediction for each SR in the given LHC analysis
    void get_all_SubGeVBeamDump_MB_signal_yields(std::vector<double>& sr_nums, const Model_analysis_info& analysis_info, const Spectrum& spec, str& modelname)
    {
      ...
      // Get the parameters we need from the theory spectrum
      // TODO: If your model has more/less than 2 masses and 2 couplings, then the get_<modelname>_signal_yields function will need to be changed.
...
/// Fill the input vector with the signal prediction for the <model> model
/// TODO: If you apply some scaling based on couplings, then this is where you would want to do this
    void get_SubGeVBeamDump_MB_signal_yields(std::vector<double>& signal_yields, const Model_analysis_info& analysis_info, double mDM, double mAp, double kappa, double gDM)
    {
      ...
      // Get the interpolator collections for the given operator_key
      Utils::interp2d_collection& signal_interp = analysis_info.get_interp2d("SubGeVBeamDump_MB_Data");
      ...
      double BR_scalarDM  = pow(gD,2)*(pow(mAp,2)-4*pow(2))/(8*M_PI*pow(mAp,2))*sqrt(pow(mAp,2)/4-pow(mdm,2)); // Branching ratio for scalar DM
      double signalcounts = signal * pow(kappa/kappa_simulated,4) * pow(gDM/gDM_simulated,2) * BR;
