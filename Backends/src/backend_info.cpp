...
namespace Gambit
{
  ...
  void Backends::backend_info::loadLibrary_C_CXX_Fortran(const str& be, const str& ver, const str& sv, bool with_BOSS)
  {
    ...
    void Backends::backend_info::loadLibrary_Python(const str& be, const str& ver, const str& sv, const str& lang)
    {
      ...
      // Bail now if the backend requires a version of Python that GAMBIT is not supporting or not configured with.
      ...
      if (lang == "Python2" or lang == "PYTHON2")
      {
        err << "Failed loading Python backend " << be << " " << ver << "." << endl
            << "GAMBIT no longer supports Python 2 backends. Please update the code to Python 3 " 
            << "and use '#define BACKENDLANG Python3' or '#define BACKENDLANG Python' in the " 
            << "frontend header file." << endl;
        backend_warning().raise(LOCAL_INFO, err.str());
        works[be+ver] = false;
