...
class PocoMC(Scanner):
    ...
    def initial_state(self):
        ...
        """
        return np.vstack([self.prior_transform(np.random.rand(self.dim)) for i in range(self.n_particles)])
    ...
    def __init__(self, n_particles=1000, vectorize_likelihood=True, **kwargs):
        ...
        self.sampler = pocomc.Sampler(
            self.n_particles, self.dim, self.loglike, self.log_prior_density, infer_vectorization=False, **kwargs)
