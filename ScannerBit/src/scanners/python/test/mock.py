...
def mock_gambit(
        ...
        name="Mock",
        log=print,
        loglike_hypercube=lambda x: -
        x[0]**2,
        prior_transform=lambda x: x,
        inverse_prior_transform=lambda x: x,
        parameter_names=None,
        log_prior_density=lambda x: 1.):
    """
    ...
    mocked = imocker.import_module('plugins')
    ...
    mocked.Scanner.inverse_prior_transform = MagicMock(
        side_effect=inverse_prior_transform)
