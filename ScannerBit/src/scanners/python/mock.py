...
@contextmanager
def mock_gambit(dim=1, mpi_rank=0, mpi_size=1, version="1.0.0", name="Mock", log=print, loglike=lambda x: -x[0]**2,
                prior_transform=lambda x: x, parameter_names=None):
    """
    ...
    mocked.Scanner.loglike = MagicMock(side_effect=loglike)
